...
Note: Apps in this repo are **not production ready**. This repo holds starters and/or hackathon results!
...
npm run start
```
The application will be available on [http://localhost:4200/](http://localhost:4200/). Once you make changes to the code, the page is refreshed automatically.

## Deploying the app on kyma Cluster
1. ### **setup the `kubectl` config for deploying on cluster**
...
2. ### **Deploying the template app on the kyma cluster**
   ...
   - Currently the deployment requires that the project is hosted in a git repo. If your project is not yet on github, initialize a new git repository for the same. Don't push your changes in the `flow-app-template` repository.
     - delete the `.git` folder
     - execute the command `git init` on the root level of the project. 
     - Open the github page where you are allowed to create repositories. For e.g. on [https://github.tools.sap/](https://github.tools.sap/)
     - click on the account in top right corner and click on `Your repositories`
     - click on new to create a new repository. Give your repository a name, and make it public or private based on your preference.
     - click `Create Repository` and follow the instructions on screen to make your new project a git repo.
   -  Adapt the `APP_NAME` to your app. You can also use the same name as your repository.
...
## BONUS: **setup local kyma cluster on macOS**
...
## More BONUS: **setup local kyma cluster on Windows**
